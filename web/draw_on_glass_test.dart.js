...
["draw_on_glass_test.dart", "draw_on_glass_test.dart", , {
main: function() {
  new $.MultiTouchDraw(null, $.List_kmp, "blue", null, $.C__Random).run$0();
},

Line: {"": "Object;x*,y*,color"},

MultiTouchDraw: {"": "Object;lines,colors,color,context,random",
  run$0: function() {
    var canvas, t1, div;
    canvas = document.querySelector("#canvas");
    t1 = $.getInterceptor$x(canvas);
    this.context = t1.getContext$1(canvas, "2d");
    div = document.querySelector("#div");
    t1.set$width(canvas, div.scrollWidth);
    t1.set$height(canvas, div.scrollHeight);
    $.set$lineWidth$x(this.context, $.JSNumber_methods.toInt$0(Math.ceil(Math.random() * 35)));
    $.set$lineCap$x(this.context, "round");
    this.lines = $.makeLiteralMap([]);
    canvas.get$onTouchStart;
    $.EventStreamProvider_touchstart.forElement$2$useCapture;
    t1 = new $._ElementEventStreamImpl(canvas, $.EventStreamProvider_touchstart._eventType, false);
    new $._EventStreamSubscription(0, t1._target, t1._eventType, this.get$draw(), t1._useCapture)._tryResume$0();
  },
  draw$1: function($event) {
    var t, id, t1, wasEmpty, t2, t3, moveX, moveY;
    $.Primitives_printString("draw: " + $.S($event));
    t = $.get$first$ax($.get$touches$x($event));
    id = t.identifier;
    t1 = this.lines;
    t1.get$isEmpty;
    wasEmpty = t1._liblib1$_length === 0;
    t2 = t.pageX;
    t3 = t.pageY;
    if (wasEmpty) {
      t1.$indexSet(t1, id, new $.Line(new $.Point(t2, t3).x, new $.Point(t2, t3).y, "blue"));
      t1 = this.lines;
      $.set$x$x(t1.$index(t1, id), new $.Point(t.pageX, t.pageY).x);
      t1 = this.lines;
      $.set$y$x(t1.$index(t1, id), new $.Point(t.pageX, t.pageY).y);
      t1 = this.lines;
      moveX = $.get$x$x(t1.$index(t1, id));
      t1 = this.lines;
      moveY = $.get$y$x(t1.$index(t1, id));
    } else {
      moveX = $.$sub$n(new $.Point(t2, t3).x, $.get$x$x(t1.$index(t1, id)));
      t1 = t.pageX;
      t2 = t.pageY;
      t3 = this.lines;
      moveY = $.$sub$n(new $.Point(t1, t2).y, $.get$y$x(t3.$index(t3, id)));
    }
    t1 = this.lines;
    $.Primitives_printString($.S(t1.$index(t1, id)) + ", " + $.S(moveX) + ", " + $.S(moveY));
    t1 = this.lines;
    this.move$3(t1.$index(t1, id), moveX, moveY);
    if (!wasEmpty) {
      t1 = this.lines;
      t1 = t1.$index(t1, id);
      t2 = this.lines;
      $.set$x$x(t1, $.$add$ns($.get$x$x(t2.$index(t2, id)), moveX));
      t2 = this.lines;
      t2 = t2.$index(t2, id);
      t1 = this.lines;
      $.set$y$x(t2, $.$add$ns($.get$y$x(t1.$index(t1, id)), moveY));
    }
  },
  get$draw: function() {
  ...
  move$3: function(line, changeX, changeY) {
    var t1;
    $.set$strokeStyle$x(this.context, this.color);
    $.beginPath$0$x(this.context);
    t1 = $.getInterceptor$x(line);
    $.moveTo$2$x(this.context, t1.get$x(line), t1.get$y(line));
    $.lineTo$2$x(this.context, $.$add$ns(t1.get$x(line), changeX), $.$add$ns(t1.get$y(line), changeY));
...
$.List_kmp = Isolate.makeConstantList(["red", "green", "yellow", "blue", "magenta", "orangered"]);
...
// Native classes
$.defineNativeMethods("CanvasGradient|CanvasPattern|DOMError|FileError|MediaError|MediaKeyError|Navigator|NavigatorUserMediaError|PositionError|SQLError|SVGAnimatedLength|SVGAnimatedLengthList|SVGAnimatedNumber|SVGAnimatedNumberList|Touch", $.Interceptor);
...
//# sourceMappingURL=draw_on_glass_test.dart.js.map
//@ sourceMappingURL=draw_on_glass_test.dart.js.map
